...
class Plotting:
    ...
    def plot_bar(self, bar_labels, y_values, y_lims, x_label:str='x', y_label:str='y', width:float=0.4, filename:str=None):
        """
    Plots a bar chart with specified labels, values, and axis limits.

    This method creates a bar chart using the provided data labels, y-values, 
    and y-axis limits. It can optionally save the plot to a specified filename.

    Parameters
    ----------
    bar_labels : list
        Labels for each bar on the x-axis.
    y_values : list or np.ndarray
        Values for each bar, representing the height of each bar.
    ...
    width : float, optional
        Width of each bar in the chart. Default is 0.4.
    filename : str, optional
        If provided, saves the plot to this filename. Default is None, meaning the plot is not saved.

    Returns
    -------
    None
        This function does not return any value. It directly displays (and/or saves) the bar plot.
    """
